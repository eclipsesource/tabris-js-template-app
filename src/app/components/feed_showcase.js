...
function appendItemBox(widget , feedItem){
	...
	if(imageUrl){
		var boxContainer = tabris.create('Composite', { left: ["prev()", 10], width: 90, top: 0, bottom: 0}).appendTo(widget);
		tabris.create('ImageView', { image:imageUrl,left: 0, right: 0, top: 0, height: 120, scaleMode: 'fill' , background: "rgb(220, 220, 220)"}).appendTo(boxContainer);
		if(feedItem.price){
...
function appendSeeAllBox(widget){
	...
	var boxContainer = tabris.create('Composite', { left: ["prev()", 10], width: 160, top: 0, bottom: 0 }).appendTo(widget);
	...
	tabris.create('TextView', { text: ''+ '' + "See all<br/>'" +feedConfig.name +"'"  , maxLines: 2, font: '16px', left: 0, right: 20, bottom: 0, top: 0, textColor: "white",  alignment:'center', markupEnabled:true}).appendTo(seeAllBox);
